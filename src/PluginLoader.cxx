...
namespace hpssim {
    ...
    SimPlugin* PluginLoader::create(std::string pluginName, std::string libName) {

        std::cout << "PluginLoader: Creating plugin '" << pluginName << "' from lib '" << libName << "'" << std::endl;

        // Open a handle to the specific dynamic lib.
        void* handle = dlopen(libName.c_str(), RTLD_LAZY);

        // Plugin to be created.
        SimPlugin* plugin = nullptr;
    ...
    void PluginLoader::destroy(SimPlugin* plugin) {

        // Is the plugin argument non-null?
        if (plugin) {

            std::cout << "PluginLoader: Destroying plugin '" << plugin->getName() << "'" << std::endl;
            ...
            if (handle) {
                ...
                void (*destroyIt)(SimPlugin*);
                destroyIt = (void (*)(SimPlugin*))dlsym(handle, std::string("destroy" + plugin->getName()).c_str());destroyIt
                (plugin);

                // Delete from plugin handles.
                std::map<SimPlugin*, void*>::iterator it;
