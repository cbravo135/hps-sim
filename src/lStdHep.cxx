...
lStdHep::lStdHep(const char *filename, bool open_for_write) :
        lXDR(filename, open_for_write), ntot(0), version(0), title(0), comment(0), date(0), closingDate(0), numevts_expect(
                0), numevts(0), firstTable(0), dimTable(0), nNTuples(0), nBlocks(0), blockIds(0), blockNames(0) {
    if (open_for_write) {
...
//
    ...
    while (1) {
        if (eventTable.ievt < eventTable.numEvts) {
            if (filePosition(eventTable.ptrEvents[eventTable.ievt]) != eventTable.ptrEvents[eventTable.ievt])
                return (getError());

            if (event.read(*this) != LSH_SUCCESS)
                return (getError());
            eventTable.ievt++;
...
//
            if (filePosition(eventTable.nextlocator) != eventTable.nextlocator)
                return (getError());
            if (eventTable.read(*this) != LSH_SUCCESS)
                return (getError());
        }
    }
    return (getError());
...
long lStdHep::getEvent(lStdEvent &lse) const {
    if (long status = getError() != LSH_SUCCESS)
        return (status);
...
lStdHep::EventTable::EventTable() :
        isEmpty(1), ievt(0), blockid(0), ntot(0), version(0), nextlocator(-3), numEvts(0), evtnums(0), storenums(0), runnums(
                0), trigMasks(0), ptrEvents(0) {
    return;
}
...
long lStdHep::EventTable::print(FILE *fp) {
    ...
    for (int i = 0; i < numEvts; i++) {
        fprintf(fp, " EventTable: %d: evtnums %ld storenums %ld runnums %ld trigMasks %ld ptrEvents %ld\n", i,
                evtnums[i], storenums[i], runnums[i], trigMasks[i], ptrEvents[i]);
        if (i == 10) {
...
lStdHep::Event::Event() :
        isEmpty(0), blockid(0), ntot(0), version(0), evtnum(0), storenum(0), runnum(0), trigMask(0), nBlocks(0), dimBlocks(
                0), nNTuples(0), dimNTuples(0), blockIds(0), ptrBlocks(0), nevhep(0), nhep(0), isthep(0), idhep(0), jmohep(
                0), jdahep(0), phep(0), vhep(0), eventweight(0), alphaqed(0), alphaqcd(0), scale(0), spin(0), colorflow(
                0), idrup(0), bnevtreq(0), bnevtgen(0), bnevtwrt(0), bstdecom(0), bstdxsec(0), bstdseed1(0), bstdseed2(
                0), enevtreq(0), enevtgen(0), enevtwrt(0), estdecom(0), estdxsec(0), estdseed1(0), estdseed2(0)
...
//
    ...
    ntot = ls.readLong();
    version = ls.readString(len);
    if (blockid != LSH_EVENTHEADER)
        ls.setError(LSH_NOEVENT);
...
long lStdHep::Event::printHeader(FILE *fp) {
    ...
    for (int i = 0; i < nBlocks; i++) {
        const char *labels[10] = { "Event", "Off-track arrays", "Off-track struct", "Trace arrays",
                "Event with multiple interactions", "Begin run", "End run", "StdHepCXX", "EventV4", "Unknown" };
        int j = blockIds[i] - 101;
        if (blockIds[i] == LSH_STDHEPEV4)
            j = 8;
        if ((j < 0) || (j > 9))
            j = 9;
