...
lXDR::lXDR(const char *filename, bool open_for_write) :
    ...
    if (htonl(1L) == 1L)
        _hasNetworkOrder = true;
    else
        _hasNetworkOrder = false;
    return;
}
...
long lXDR::writeString(const char *data, long length) {
    if (checkWrite(&length))
        return (_error);
    if (fwrite(data, 1, length, _fp) != (unsigned long) length)
        return (_error = LXDR_WRITEERROR);
    long l = ((length + 3) & 0xFFFFFFFC) - length;
    if (fwrite(&l, 1, l, _fp) != (unsigned long) l)
        return (_error = LXDR_WRITEERROR);
